...
async def translate_text(session, text):
    ...
    try:
        ...
        logger.info(f"Yandex translate request: '{text[:50]}...'")
        async with session.post(YANDEX_TRANSLATE_URL, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            ...
            logger.info(f"Yandex translate status: {resp.status}")
            if resp.status == 200:
                ...
                if translations:
                    ...
                    logger.info(f"Yandex translate success: '{text[:30]}...' -> '{result[:30]}...'")
                    return result
                else:
                    logger.warning(f"Yandex translate: no translations for '{text[:50]}...'")
            else:
                logger.error(f"Yandex translate error {resp.status}: {response_text[:200]}")
    except Exception as e:
        logger.error(f"Yandex translation failed: {e}")
    return text
        ...
        async with session.post(YANDEX_TRANSLATE_URL, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=15)) as resp:
            ...
            if resp.status == 200:
                ...
                for idx, trans in zip(indices, translations):
                    ...
                    texts[idx] = result
                logger.info(f"Yandex batch success: {len(translations)} translations")
            else:
                logger.error(f"Yandex batch error {resp.status}: {response_text[:200]}")
    except Exception as e:
        logger.error(f"Yandex batch translation failed: {e}")
    return texts
...
async def button_handler_help(update, context):
    """Handler for 'help' button - works both inside and outside conversation"""
    query = update.callback_query
    await query.answer()
...
async def button_handler_main_menu(update, context):
    """Handler for 'main_menu' button - works both inside and outside conversation"""
    query = update.callback_query
    await query.answer()
...
async def main():
    ...
    # === CONVERSATION HANDLER for "find recipe by ingredients" flow ===
    # Entry points: buttons that START the conversation
    # States: handlers active DURING the conversation
    # Fallbacks: handlers that work when conversation is active but no state matched
    conv_handler = ConversationHandler(
        ...
        fallbacks=[
            ...
            # These buttons work as fallbacks when conversation is active
            CallbackQueryHandler(random_recipe, pattern=r"^random_recipe$"),
            CallbackQueryHandler(button_handler_main_menu, pattern=r"^main_menu$"),
            CallbackQueryHandler(button_handler_help, pattern=r"^help$"),
            CallbackQueryHandler(find_recipe_start, pattern=r"^find_recipe$"),
        ],
    )
    ...
    # === STANDALONE HANDLERS for when conversation is NOT active ===
    # These must be AFTER conv_handler so they catch callbacks conv_handler doesn't handle
    # (when conversation is not active)
    application.add_handler(CallbackQueryHandler(random_recipe, pattern=r"^random_recipe$"))
    application.add_handler(CallbackQueryHandler(button_handler_help, pattern=r"^help$"))
    application.add_handler(CallbackQueryHandler(button_handler_main_menu, pattern=r"^main_menu$"))
    ...
    if WEBHOOK_URL:
        ...
        await site.start()
        logger.info(f"✅ Webhook запущен на {full_webhook_url} (порт {PORT})")
